...
type McpHttpSmokeTests(output: ITestOutputHelper) =
    ...
    let ``MCP server responds to status tool calls`` () =
        async {
            ...
            let clientTransport = new SseClientTransport(new SseClientTransportOptions(Endpoint = client.BaseAddress), client, null, false);
            let! mcpClient = McpClientFactory.CreateAsync(clientTransport) |> Async.AwaitTask

            let! tools = mcpClient.ListToolsAsync().AsTask() |> Async.AwaitTask
            tools |> Seq.iter(fun tool -> output.WriteLine($"{tool.Name}"))
            test <@ tools |> Seq.exists (fun tool -> tool.Name = McpToolNames.GetFsiStatus) @>
            
            let! statusResult = (mcpClient.CallToolAsync(McpToolNames.GetFsiStatus, null)).AsTask() |> Async.AwaitTask
            let statusText = (statusResult.Content[0] :?> ModelContextProtocol.Protocol.TextContentBlock).Text
    ...
    let ``MCP HTTP round trip - SendFSharpCode and GetRecentFsiEvents`` () =
        async {
            ...
            let! codeResponse = client.PostAsync("/message", codeContent) |> Async.AwaitTask
            let! codeResponseString = codeResponse.Content.ReadAsStringAsync() |> Async.AwaitTask
            ...
            let! eventsResponse = client.PostAsync("/message", eventsContent) |> Async.AwaitTask
            let! eventsResponseString = eventsResponse.Content.ReadAsStringAsync() |> Async.AwaitTask
